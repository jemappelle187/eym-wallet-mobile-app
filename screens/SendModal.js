...
const SendModal = ({ visible, onClose, currentCurrency, currentBalance, onCurrencyChange }) => {
  ...
  const getCurrentRate = (currencyCode) => {
    // For now, return null since we don't have live rates in send modal
    // This matches the add money modal behavior when rates are not available
    return null;
  };
  ...
  // Handle currency change with fallback
  const handleCurrencyChange = (currencyCode) => {
    if (typeof onCurrencyChange === 'function') {
      onCurrencyChange(currencyCode);
    }
    // If no callback provided, just close the dropdown
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.modalHeader}>
            <TouchableOpacity 
              ...
              onPress={onClose}
              accessibilityLabel="Close send money modal"
              accessibilityHint="Double tap to close the send money screen"
                          ...
                          <View style={styles.cardStackCardContent}>
                            ...
                            <TouchableOpacity
                              onPress={(e) => {
                                e.stopPropagation();
                                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                                setCurrentCardIndex(prev => prev === 0 ? sendOptions.length - 1 : prev - 1);
                              }}
                              activeOpacity={0.7}
                            ...
                            <TouchableOpacity
                              onPress={(e) => {
                                e.stopPropagation();
                                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                                setCurrentCardIndex(prev => prev === sendOptions.length - 1 ? 0 : prev + 1);
                              }}
                              activeOpacity={0.7}
