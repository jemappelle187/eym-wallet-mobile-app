...
const groupTransactionsByDate = (transactions) => {
  ...
  transactions.forEach(tx => {
    ...
    } else if (tx >= startOfWeek) {
      groups['This Week'].push(tx);
    } else if (tx >= startOfMonth) {
...
const HomeScreen = () => {
  ...
  console.log('colors in HomeScreen_calculator_fix_backup:', colors);
  ...
  const competitorData = [
    {
      name: 'Western Union',
      logo: '🏦',
      rate: 0.075, // Worse rate (converts less USD for same input)
      fee: 12.50,
      total: 0,
    },
    {
      name: 'MoneyGram',
      logo: '💳',
      rate: 0.077, // Worse rate (converts less USD for same input)
      fee: 15.00,
      total: 0,
    },
    {
      name: 'SendNReceive',
      logo: '⭐',
      rate: 0.083, // Best rate (our rate - converts more USD for same input)
      fee: 0.00, // No fees
      total: 0,
    },
  ];
  ...
  useEffect(() => {
    if (localAmount && !isNaN(Number(localAmount)) && selectedCurrencyObj) {
      const amount = Number(localAmount);
      competitorData.forEach(comp => {
        // Calculate what user gets in target currency (e.g., GHS) after conversion and fees
        // First convert the input amount to USD using the competitor's rate
        const usdEquivalent = amount * comp.rate;
        // Then subtract the fee
        const afterFees = usdEquivalent - comp.fee;
        // Convert back to target currency (e.g., GHS) using the inverse rate
        comp.total = afterFees / selectedCurrencyObj.rateToUSDC;
      });
    }
  }, [localAmount, selectedCurrencyObj]);
      ...
      >
        ...
        <View style={{opacity: 0.88}}>
          ...
          <View style={[styles.calculatorCard, { backgroundColor: '#f8fafc' }]}>
            <Text style={[styles.calculatorTitle, { color: '#1e293b' }]}>Currency Calculator & Rate Comparison</Text>
            ...
            <View style={styles.competitorContainer}>
              <Text style={[styles.competitorSectionTitle, { color: '#1e293b', marginBottom: 12 }]}>Compare with Competitors</Text>
              {competitorData.map((competitor, index) => (
                                  <View key={competitor.name} style={[
                    styles.competitorItem,
                    competitor.name === 'SendNReceive' && styles.ourService
                  ]}>
                    <View style={styles.competitorHeader}>
                      <Text style={styles.competitorLogo}>{competitor.logo}</Text>
                      <Text style={[
                        styles.competitorName,
                        competitor.name === 'SendNReceive' && styles.ourServiceText
                      ]}>{competitor.name}</Text>
                      {competitor.name === 'SendNReceive' && (
                  ...
                  <View style={styles.competitorDetails}>
                    <View style={styles.competitorDetail}>
                      <Text style={styles.competitorDetailLabel}>Rate:</Text>
                      <Text style={styles.competitorDetailValue}>{competitor.rate.toFixed(3)}</Text>
                    </View>
                    <View style={styles.competitorDetail}>
                      <Text style={styles.competitorDetailLabel}>Fee:</Text>
                      <Text style={[
                        styles.competitorDetailValue,
                        competitor.fee === 0 && styles.noFeeText
                      ]}>
                        {competitor.fee === 0 ? 'FREE' : `$${competitor.fee.toFixed(2)}`}
                      </Text>
                    </View>
                    <View style={styles.competitorDetail}>
                      <Text style={styles.competitorDetailLabel}>You Get ({localCurrency}):</Text>
                      <Text style={[
                        styles.competitorDetailValue,
                        styles.totalAmount,
                        competitor.name === 'SendNReceive' && styles.ourServiceText
                      ]}>
                        {selectedCurrencyObj?.symbol || ''}{competitor.total.toFixed(2)}
                      </Text>
                    </View>
                  </View>
                </View>
